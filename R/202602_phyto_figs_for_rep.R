...
# cell abundance per unit volume,
# estimated cell volume,
# carbon content per cell (derived from mean & median carbon values when
# a taxon fits > entries on Angus' & Tom's table)
# these mean & median carbon values are multiplied by cell densities to give
# carbon per unit volume.  These include carbon contents derived from both
# mean & median carbon contents per cell.
# Plots & summaries are based on the total carbon values in each sample, with
# values a result of summing the individual carbon contents of each taxon per sample
# The carbon values used are derived from the median carbon values per taxon
# (when the table) contains >1 entry for a given taxon
df %>% #names()
  ## total carbon: cells per litre
  select(sample_id, date_plot, md_carbon_tot_ug_c_per_m3, rbd) %>% 
  ##remove zero values
  ...
  summarise(md_carbon_tot_ug_c_per_m3 = median(md_carbon_tot_ug_c_per_m3,na.rm=TRUE),
            ...
            .groups = "drop") %>% ungroup() %>% 
  group_by(rbd) %>% 
  summarize(min = min(md_carbon_tot_ug_c_per_m3, na.rm = TRUE),
            q1 = quantile(md_carbon_tot_ug_c_per_m3,0.25, na.rm = TRUE),
            median = median(md_carbon_tot_ug_c_per_m3, na.rm = TRUE),
            mean = mean(md_carbon_tot_ug_c_per_m3, na.rm = TRUE),
            q3 = quantile(md_carbon_tot_ug_c_per_m3,0.75, na.rm = TRUE),
            max=max(md_carbon_tot_ug_c_per_m3, na.rm = TRUE)) %>% 
  write.csv(.,
            file = paste0(outfol,"md_C_ug_per_m3_summary_by_rbd.csv"),
            row.names = FALSE)
...
df %>% #names()
  ...
  # select(-sample_id) %>% 
  write.csv(.,
            file = paste0(outfol,"median_C_ug_per_m3_by_rbd.csv"),
            row.names = FALSE)

write.csv(df,
          file = paste0(outfol,"md_C_ug_per_m3_raw.csv"),
          row.names = FALSE)
